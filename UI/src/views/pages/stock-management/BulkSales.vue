...
const uploadFile = async () => {
    ...
    formData.append("date", commonService.formatDate(date.value));
    // formData.append("date", Math.floor(date.value.getTime() / 1000));
    try {
        const response = await fetch(commonService.baseUrl+'upload-sales', {
            method: 'POST',
            body: formData,
            headers: {
        ...
        router.push("/view-uploaded-sales");
    } catch (error) {
...
<template>
    <div class="card">
        <h5>Bulk Upload</h5>
        <a href="demo/documents/bulk_sales_template.xlsx">
