...
const getMemberBatch=()=>{
    commonService.genericRequest('get-sales-uploaded-batch', 'post', true, {}).then((response) => {
        if (response.status) {
...
const goToApprove=(event)=>{
    router.push("/approve-uploaded-sales/"+event?.id);
}
...
<template>
    <div className="card">
        <h5>View Bulk Uploaded Sales</h5>
