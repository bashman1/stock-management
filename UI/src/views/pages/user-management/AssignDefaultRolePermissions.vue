...
const getRoleDetails = (id) => {
    commonService.genericRequest('get-default-role/' + id, 'get', true, {}).then((response) => {
        if (response.status) {
...
const onChangeSwitch = (event, permission) => {
    ...
    commonService.genericRequest("assign-default-role-permission", "post", true, postObject).then(
        (response) => {
            if (response.status) {
            }
        }
